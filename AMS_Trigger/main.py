"""
AMS_Trigger Main is the main module that trigger all the other AMS Groups based on the ProfileID and BusinessGroup.
"""
import os
import sys
import shutil
...
### 1 Call XML builder by running 'get_agreements()'
...
### 3 - Updating the WFDocuSing Table
for file in os.scandir(output):
	profile_id = Path(file).stem.split('_')
	updateProcessTable(profile_id[1], current_status='NEW', new_status='XML Format Completed')
	print(f'ProfileID: {profile_id[1]} Status updated in Process Table')
